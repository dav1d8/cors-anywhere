...
function getHandler(options, proxy) {
  ...
  return async function(req, res) {
    ...
    //Fix for nginx proxy normalization that does not allow double slashes
    ...
    var location = parseURL(_url);

    if (corsAnywhere.handleInitialRequest && await corsAnywhere.handleInitialRequest(req, res, location, options)) {
      return;
    }
...
exports.createServer = function createServer(options) {
  ...
  server.on("upgrade", function (req, socket, head) {
    let requestUrl = req.url.slice(1);
    if (!requestUrl.includes("ws://") && !requestUrl.includes("wss://")) {
      requestUrl = requestUrl.replace("ws:/", "ws://");
      requestUrl = requestUrl.replace("wss:/", "wss://");
    }
    req.url = requestUrl;
    ...
    (options.removeWsHeaders ?? []).forEach(function (header) {
      delete req.headers[header];
    });
